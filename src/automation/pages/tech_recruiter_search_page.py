...
class TechRecruiterSeachPage:
    ...
    def navigate(self) -> bool:
        ...
        try:
            ...
            logger.error(f"{self.PAGE_NAME} - Não foi possível navegar para a página solicitada - {str(e)}")
            return False
    
    def _get_recruiter_list(self):
        time.sleep(1)
    ...
    def get_connectables_list(self) -> list[WebElement]:
        try:
            ...
            logger.error(f"{self.PAGE_NAME} - Erro ao pegar lista de recrutadores - {str(e)}")
            return []
    ...
    def try_connect(self, recruiter: WebElement):
        try:
            ...
            logger.error(f"{self.PAGE_NAME} - Erro ao tentar conectar com recrutador - {str(e)}")
            return False
    ...
    def _try_connect_confirmation(self):
        try:
            ...
            modal = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.ID, "artdeco-modal-outlet"))  # Usando EC corretamente
            )
            ...
            logger.error(f"{self.PAGE_NAME} - Erro ao tentar confirmar conexão com recrutador - {str(e)}")
            raise e
            return False